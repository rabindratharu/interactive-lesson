...
/**
 * Plugin Utils for Interactive Lesson.
 *
 ...
 * Utils class.
 *
 * Handles utility functions for the Interactive Lesson plugin.
 *
 * @since 1.0.0
 */
class Utils
{
    ...
    public static function get_posts($args): array
    {
        ...
        if ($query->have_posts()) {
            while ($query->have_posts()) {
                $query->the_post();
                $items[get_the_ID()] = get_the_title() ?: __('(no title)', 'interactive-lesson');
            }
        }
    ...
    {
        $defaults = [
            'setting1'  => esc_html__('Default Setting 1', 'interactive-lesson'),
            'blocks'  => esc_html__('Default Setting 2', 'interactive-lesson'),
            'setting3'  => false,
            'quiz_block'  => true,
            'setting5'  => 'option-1',
            'deleteAll' => false,
        ...
        return apply_filters(INTERACTIVE_LESSON_NAME . '_get_default_options', $defaults);
    }
    ...
    {
        if (!defined('INTERACTIVE_LESSON_NAME')) {
            return $key ? false : [];
        }

        $options = get_option(INTERACTIVE_LESSON_NAME, []);
    ...
    public static function update_options($key_or_data, $val = ''): void
    {
        ...
        update_option(INTERACTIVE_LESSON_NAME, $options);
    }
    ...
    public static function file_system()
    {
        ...
        if (!defined('ABSPATH')) {
            return new \WP_Error('filesystem_error', __('ABSPATH is not defined.', 'interactive-lesson'));
        }
        ...
        if (!$wp_filesystem) {
            ...
            if (!WP_Filesystem()) {
                ...
                return new \WP_Error('filesystem_error', __('Failed to initialize the WordPress filesystem.', 'interactive-lesson'));
            }
        }
    ...
    public static function parse_changelog(): string
    {
        ...
        $changelog_file = apply_filters(INTERACTIVE_LESSON_NAME . '_changelog_file', defined('INTERACTIVE_LESSON_PATH') ? INTERACTIVE_LESSON_PATH . 'readme.txt' : '');

        if (empty($changelog_file) || !$wp_filesystem->exists($changelog_file) || !$wp_filesystem->is_readable($changelog_file)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('Interactive_Lesson: Changelog file not found or not readable at ' . $changelog_file);
            }
            return '';
        }
    ...
    public static function get_settings_schema(): array
    {
        ...
        $setting_properties = apply_filters(
            INTERACTIVE_LESSON_NAME . '_options_properties',
            [
                'setting1' => [
                    'type'        => 'string',
                    'description' => __('First text setting.', 'interactive-lesson'),
                    'default'     => $defaults['setting1'],
                ],
                'blocks' => [
                    'type'        => 'string',
                    'description' => __('Second text setting.', 'interactive-lesson'),
                    'default'     => $defaults['blocks'],
                ],
                'setting3' => [
                    'type'        => 'boolean',
                    'description' => __('First boolean setting.', 'interactive-lesson'),
                    'default'     => $defaults['setting3'],
                ],
                'quiz_block' => [
                    'type'        => 'boolean',
                    'description' => __('Second boolean setting.', 'interactive-lesson'),
                    'default'     => $defaults['quiz_block'],
                ],
                'setting5' => [
                    'type'        => 'string',
                    'description' => __('Option selection setting.', 'interactive-lesson'),
                ...
                'deleteAll' => [
                    'type'        => 'boolean',
                    'description' => __('Delete all settings on plugin deactivation.', 'interactive-lesson'),
    ...
    public static function white_label($key = '')
    {
        $plugin_name = apply_filters(
            'interactive_lesson_white_label',
            esc_html__('Interactive Lesson', 'interactive-lesson')
        );

        $options = apply_filters(
            'interactive_lesson_white_label_options',
            [
                'admin_menu_page' => [
                    'page_title' => esc_html__('Interactive Lesson Page', 'interactive-lesson'),
                    'menu_title' => esc_html__('Interactive Settings', 'interactive-lesson'),
                    'menu_slug'  => INTERACTIVE_LESSON_NAME,
                    'icon_url'   => INTERACTIVE_LESSON_BUILD_PATH_URL . '/images/placeholder.png',
                    'position'   => null,
                ],
                'dashboard' => [
                    'logo'   => INTERACTIVE_LESSON_BUILD_PATH_URL . '/images/placeholder.png',
                    'notice' => sprintf(
                        /* translators: %s is the plugin name */
                        esc_html__('Congratulations on choosing the %s for creating your plugin. We recommend taking a few minutes to read the following information on how the plugin works. Please read it carefully to fully understand the capabilities of the plugin and how to use them effectively.', 'interactive-lesson'),
                        $plugin_name
                    ),
                ],
                'landingPage' => [
                    'banner' => [
                        ...
                        'leadText'   => sprintf(
                            /* translators: %s is the plugin name */
                            esc_html__('Congratulations! You have successfully installed %s and it is ready for customization. Feel free to add/edit any files.', 'interactive-lesson'),
                            $plugin_name
                        ),
                        'normalText' => sprintf(
                            /* translators: %s is the plugin name */
                            esc_html__('If you have any questions or need assistance, please do not hesitate to contact us for support. The %s plugin caters to WordPress developers and designers seeking to quickly start developing WordPress plugins in a modern way using ReactJS, Rest API.', 'interactive-lesson'),
                            $plugin_name
                        ),
                        'buttons' => [
                            [
                                ...
                                'text'    => esc_html__('Get started', 'interactive-lesson'),
                                'url'     => 'https://github.com/rabindratharu/interactive-lesson',
                                'variant' => 'primary',
                            ],
                            [
                                ...
                                'text'    => esc_html__('Documentation', 'interactive-lesson'),
                                'url'     => 'https://github.com/rabindratharu/interactive-lesson',
        ...
        5.123-6.83-2.928.002z"/></svg>',
                                'text'    => esc_html__('Get support', 'interactive-lesson'),
                                'url'     => 'https://github.com/rabindratharu/interactive-lesson',
                                'variant' => 'secondary',
                            ],
                        ],
                        'image' => INTERACTIVE_LESSON_BUILD_PATH_URL . '/images/placeholder.png',
                    ],
                    'identity' => [
                        'logo'    => INTERACTIVE_LESSON_BUILD_PATH_URL . '/images/placeholder.png',
                        'title'   => $plugin_name,
                        'buttons' => [
                            [
                                'text'    => esc_html__('Visit site', 'interactive-lesson'),
                                'url'     => 'https://github.com/rabindratharu/interactive-lesson',
                                'variant' => 'primary',
                            ],
                            [
                    ...
                    'contact' => [
                        'title' => esc_html__('Contact Information', 'interactive-lesson'),
                        'info'  => [
                            [
                                ...
                                'title'   => esc_html__('Support', 'interactive-lesson'),
                                'text'    => esc_html__('Get Support', 'interactive-lesson'),
                                'url'     => 'https://github.com/rabindratharu/interactive-lesson',
                                'variant' => 'link',
                            ],
                            [
                                ...
                                'title'   => esc_html__('Email', 'interactive-lesson'),
                                'text'    => esc_html__('rabindra.tharu.np@gmail.com', 'interactive-lesson'),
                            ...
                            [
                                ...
                                'title' => esc_html__('Location', 'interactive-lesson'),
                                'text'  => esc_html__('Kathmandu, Nepal', 'interactive-lesson'),
                            ],
                        ],
                        'social' => [
                            [
                    ...
                    'bannerColumns' => [
                        [
                            ...
                            'title' => esc_html__('Activate Plugin', 'interactive-lesson'),
                        ],
                        [
                            ...
                            'title' => esc_html__('Login settings', 'interactive-lesson'),
                        ],
                        [
                            ...
                            'title' => esc_html__('Dashboard settings', 'interactive-lesson'),
                        ],
                    ],
                    'normalColumns' => [
                        [
                            ...
                            'title'      => esc_html__('Knowledge base', 'interactive-lesson'),
                            'content'    => esc_html__('The utilization of this plugin can be facilitated by perusing comprehensive and well-documented articles.', 'interactive-lesson'),
                            'buttonText' => esc_html__('Visit knowledge base', 'interactive-lesson'),
                            'buttonLink' => 'https://github.com/rabindratharu/interactive-lesson',
                        ],
                        [
                            'icon'       => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="at-svg" viewBox="0 0 16 16"><path d="M14 1a1 1 0 0 1 1 1v8a1 1 0 0 1-1 1H4.414A2 2 0 0 0 3 11.586l-2 2V2a1 1 0 0 1 1-1zM2 0a2 2 0 0 0-2 2v12.793a.5.5 0 0 0 .854.353l2.853-2.853A1 1 0 0 1 4.414 12H14a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2z"/><path d="M3 3.5a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9a.5.5 0 0 1-.5-.5M3 6a.5.5 0 0 1 .5-.5h9a.5.5 0 0 1 0 1h-9A.5.5 0 0 1 3 6m0 2.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5"/></svg>',
                            'title'      => esc_html__('Community', 'interactive-lesson'),
                            'content'    => sprintf(
                                /* translators: %s is the plugin name */
                                esc_html__('Our objective is to enhance the customer experience, we invite you to join our community where you can receive immediate support.', 'interactive-lesson'),
                                $plugin_name
                            ),
                            'buttonText' => esc_html__('Visit community page', 'interactive-lesson'),
                            'buttonLink' => 'https://github.com/rabindratharu/interactive-lesson',
                        ],
                        [
                            ...
                            'title'      => esc_html__('24x7 support', 'interactive-lesson'),
                            'content'    => sprintf(
                                /* translators: %s is the plugin name */
                                esc_html__('Our support team is available 24/7 to assist you in the event that you encounter any problems while utilizing this plugin.', 'interactive-lesson'),
                                $plugin_name
                            ),
                            'buttonText' => esc_html__('Create a support thread', 'interactive-lesson'),
                            'buttonLink' => 'https://github.com/rabindratharu/interactive-lesson',
                        ],
                        [
                            ...
                            'title'      => esc_html__('Video guide', 'interactive-lesson'),
                            'content'    => sprintf(
                                /* translators: %s is the plugin name */
                                esc_html__('The plugin is accompanied by comprehensive video tutorials that provide practical demonstrations for most customization.', 'interactive-lesson'),
                                $plugin_name
                            ),
                            'buttonText' => esc_html__('View video guide', 'interactive-lesson'),
                            'buttonLink' => 'https://github.com/rabindratharu/interactive-lesson',
                        ],
                    ],
                    'topicLinks' => [
                        ...
                        'title'   => esc_html__('Quick links to settings', 'interactive-lesson'),
                        'columns' => [
                            [
                                'icon'    => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="at-svg" viewBox="0 0 16 16"><path d="M1 2.828c.885-.37 2.154-.769 3.388-.893 1.33-.134 2.458.063 3.112.752v9.746c-.935-.53-2.12-.603-3.213-.493-1.18.12-2.37.461-3.287.811zm7.5-.141c.654-.689 1.782-.886 3.112-.752 1.234.124 2.503.523 3.388.893v9.923c-.918-.35-2.107-.692-3.287-.81-1.094-.111-2.278-.039-3.213.492zM8 1.783C7.015.936 5.587.81 4.287.94c-1.514.153-3.042.672-3.994 1.105A.5.5 0 0 0 0 2.5v11a.5.5 0 0 0 .707.455c.882-.4 2.303-.881 3.68-1.02 1.409-.142 2.59.087 3.223.877a.5.5 0 0 0 .78 0c.633-.79 1.814-1.019 3.222-.877 1.378.139 2.8.62 3.681 1.02A.5.5 0 0 0 16 13.5v-11a.5.5 0 0 0-.293-.455c-.952-.433-2.48-.952-3.994-1.105C10.413.809 8.985.936 8 1.783"/></svg>',
                                'title'   => esc_html__('Settings 1', 'interactive-lesson'),
                            ...
                            [
                                'icon'    => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="at-svg" viewBox="0 0 16 16"><path d="M1 2.828c.885-.37 2.154-.769 3.388-.893 1.33-.134 2.458.063 3.112.752v9.746c-.935-.53-2.12-.603-3.213-.493-1.18.12-2.37.461-3.287.811zm7.5-.141c.654-.689 1.782-.886 3.112-.752 1.234.124 2.503.523 3.388.893v9.923c-.918-.35-2.107-.692-3.287-.81-1.094-.111-2.278-.039-3.213.492zM8 1.783C7.015.936 5.587.81 4.287.94c-1.514.153-3.042.672-3.994 1.105A.5.5 0 0 0 0 2.5v11a.5.5 0 0 0 .707.455c.882-.4 2.303-.881 3.68-1.02 1.409-.142 2.59.087 3.223.877a.5.5 0 0 0 .78 0c.633-.79 1.814-1.019 3.222-.877 1.378.139 2.8.62 3.681 1.02A.5.5 0 0 0 16 13.5v-11a.5.5 0 0 0-.293-.455c-.952-.433-2.48-.952-3.994-1.105C10.413.809 8.985.936 8 1.783"/></svg>',
                                'title'   => esc_html__('Settings 2', 'interactive-lesson'),
                            ...
                            [
                                'icon'    => '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="at-svg" viewBox="0 0 16 16"><path d="M1 2.828c.885-.37 2.154-.769 3.388-.893 1.33-.134 2.458.063 3.112.752v9.746c-.935-.53-2.12-.603-3.213-.493-1.18.12-2.37.461-3.287.811zm7.5-.141c.654-.689 1.782-.886 3.112-.752 1.234.124 2.503.523 3.388.893v9.923c-.918-.35-2.107-.692-3.287-.81-1.094-.111-2.278-.039-3.213.492zM8 1.783C7.015.936 5.587.81 4.287.94c-1.514.153-3.042.672-3.994 1.105A.5.5 0 0 0 0 2.5v11a.5.5 0 0 0 .707.455c.882-.4 2.303-.881 3.68-1.02 1.409-.142 2.59.087 3.223.877a.5.5 0 0 0 .78 0c.633-.79 1.814-1.019 3.222-.877 1.378.139 2.8.62 3.681 1.02A.5.5 0 0 0 16 13.5v-11a.5.5 0 0 0-.293-.455c-.952-.433-2.48-.952-3.994-1.105C10.413.809 8.985.936 8 1.783"/></svg>',
                                'title'   => esc_html__('Advanced settings', 'interactive-lesson'),
                    ...
                    'changelog' => [
                        ...
                        'title'   => esc_html__('Changelog', 'interactive-lesson'),
